...
namespace UnityCommons {
    public static class Rand {
        ...
        public static int Seed {
            set {
                if (stateStack.Count == 0)
                    Console.WriteLine("Modifying the initial rand seed. Call PushState() first. The initial rand seed should always be based on the startup time and set only once.");
        ...
        public static void EnsureStateStackEmpty() {
            if (stateStack.Count <= 0)
                return;
            Console.WriteLine("Random state stack is not empty. There were more calls to PushState than PopState. Fixing.");
